...
describe('encryption test(s)', function() {
    ...
    test_it(suiteName + ' Open encrypted DB with another password - should fail', function () {
      ...
      it(suiteName + ' Attempt to open encrypted DB with INCORRECT password THEN OPEN & READ with CORRECT PASSWORD [TBD PLUGIN BROKEN: ___]', function (done) {
                ...
                window.sqlitePlugin.openDatabase({name: dbName, key: 'another-password', location: 'default'}, function (db2) {
                  ...
                  window.sqlitePlugin.openDatabase({name: dbName, key: 'test-password', location: 'default'}, function (db3) {
                    ...
                    db3.transaction(function(tx) {
                      ...
                      // TEST GETS HERE Android/iOS:
                      //expect(false).toBe(true);
                      //expect(error.message).toBe('--');
                      expect(error).toBeDefined();
                      db3.close(function() {
                        expect('PLUGIN BEHAVIOR CHANGED PLEASE UPDATE THIS TEST AND TRY AGAIN TO CHECK DATA').toBe('--');
                        done();
                      }, function(error) {
                        // TBD ???:
                        //expect(error).toBeDefined();
                    ...
                    // */
                    /* ** FUTURE TBD ALT 2 [NO SQL CALLBACK]:
